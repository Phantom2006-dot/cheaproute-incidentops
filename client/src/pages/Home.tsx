...
export default function Home() {
  ...
  const simulate = trpc.incidentOps.simulate.useMutation();
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-[#f4f7fb] text-slate-950">
      ...
      <main className="relative mx-auto max-w-7xl px-5 py-6 sm:px-8 lg:px-10">
        ...
        <section className="mt-6"><Card className="border-slate-200/80 bg-slate-950 text-white shadow-sm"><CardHeader><CardTitle className="flex items-center gap-2 text-base"><Radio className="h-4 w-4 text-indigo-300" /> Verified MuJoCo episode</CardTitle><CardDescription className="text-slate-400">Runs the real seeded simulator when the robotics runtime is installed. Approval is required before execution.</CardDescription></CardHeader><CardContent className="space-y-4"><Button type="button" className="bg-white text-slate-950 hover:bg-indigo-100" disabled={simulate.isPending} onClick={() => simulate.mutate({ approve: true })}>{simulate.isPending ? "Running simulator…" : "Approve and run MuJoCo episode"}<ArrowUpRight className="ml-2 h-4 w-4" /></Button>{simulate.data ? <div className={`rounded-xl p-4 text-sm ${simulate.data.status === "PASS" ? "bg-emerald-400/15 text-emerald-100" : "bg-amber-400/15 text-amber-100"}`}><p className="font-semibold">{simulate.data.status} · {simulate.data.message ?? `${simulate.data.result?.evaluation.completion_ratio === 1 ? "100% completion" : "Simulation result returned"}`}</p><p className="mt-1 text-xs text-slate-300">{simulate.data.compatibility}</p>{simulate.data.result ? <p className="mt-2 text-xs text-slate-300">Run {simulate.data.result.run_id} · {simulate.data.result.evaluation.collision_count} collisions · {simulate.data.result.events.length} trace events · {simulate.data.result.camera_observation.camera} camera</p> : null}</div> : null}{simulate.error ? <p className="text-sm text-rose-300">{simulate.error.message}</p> : null}</CardContent></Card></section>
